...
export default function GuardianEventDetailPage() {
    ...
    const fetchChildren = async () => {
        try {
            ...
            const res = await api.get('/users/me/linked-youth/');
    ...
    const fetchChildRegistrations = async () => {
        ...
        try {
            ...
            const res = await api.get(`/events/${params.id}/registrations/`);
    ...
    if (loading || !event) {
        return (
            <div className="min-h-screen bg-[var(--dark-900)]">
                <GuardianNavBar darkMode={true} onMenuToggle={() => setIsSidebarOpen(!isSidebarOpen)} />
        ...
        <div className="min-h-screen bg-[var(--dark-900)]">
                        ...
                        <div className="max-w-7xl mx-auto">
                            ...
                            <div className="flex flex-wrap gap-2 mb-3">
                                ...
                                {registeredChildren.length > 0 && (
                                    <span className="bg-[var(--brand-primary)] text-[var(--dark-900)] text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1.5">
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    {registeredChildren.length > 0 && (
                                        <div className="space-y-2">
                                            <p className="text-xs text-[var(--brand-light)]/50 uppercase tracking-wider font-medium">{t('registeredChildren') || 'Registered Children'}</p>
                                            {registeredChildren.map(child => (
                                                <div key={child.id} className="flex items-center justify-between p-3 bg-[var(--brand-green)]/10 rounded-xl border border-[var(--brand-green)]/20">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-sm font-medium text-[var(--brand-light)]">{child.first_name}</span>
                                                    </div>
                                                    <button
                                                        onClick={() => { setChildToCancel(child); setCancelModalOpen(true); }}
                                                        className="text-xs text-[var(--brand-light)]/50 hover:text-[var(--brand-red)] transition-colors"
                                                    >
                                                        {t('cancel')}
                                                    </button>
                                                </div>
                                    ...
                                    {!event.allow_registration && (
                                        <div className="text-center py-3 text-[var(--brand-light)]/60 text-sm">
                ...
                <div className="flex items-center gap-3 max-w-lg mx-auto">
                    ...
                    <div className="flex-1">
                        {event.allow_registration && unregisteredChildren.length > 0 && !isRegistrationClosed ? (
                            ...
                            <div className="bg-[var(--brand-primary)]/20 border border-[var(--brand-primary)]/30 text-[var(--brand-primary)] font-bold py-3 rounded-xl text-center text-sm flex items-center justify-center gap-1">
            ...
            {isRegModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border border-[var(--dark-600)] p-6 max-w-md w-full animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-[var(--brand-light)] font-heading">
                                {t('selectChild') || 'Select a Child'}
                            </h3>
                            <button 
                                onClick={() => { setRegModalOpen(false); setSelectedChild(null); }}
                                className="p-2 rounded-lg text-[var(--brand-light)]/60 hover:bg-[var(--dark-700)]"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        
                        <p className="text-[var(--brand-light)]/60 mb-4 text-sm">
                            ...
                            {unregisteredChildren.map(child => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 text-left">
                                        <p className="font-semibold text-[var(--brand-light)]">{child.first_name} {child.last_name}</p>
                                    </div>
                                    {selectedChild?.id === child.id && (
                        ...
                        <div className="flex gap-3">
                            <button 
                                onClick={() => { setRegModalOpen(false); setSelectedChild(null); }}
                                className="flex-1 py-3 px-4 font-semibold text-[var(--brand-light)]/70 bg-[var(--dark-700)] rounded-xl hover:bg-[var(--dark-600)] transition-all"
                            >
                                {t('cancel')}
                            </button>
                            <button 
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border border-[var(--dark-600)] p-8 max-w-md w-full animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <p className="text-[var(--brand-light)]/70 mb-8 text-center leading-relaxed">
                            {t('cancelChildRegistration') || `Are you sure you want to cancel ${childToCancel.first_name}'s registration for`} <span className="font-semibold text-[var(--brand-light)]">{event?.title}</span>?
                        </p>
