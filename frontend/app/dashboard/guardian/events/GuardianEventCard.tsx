...
interface GuardianEventCardProps {
    event: Event;
    darkMode?: boolean;
}

export default function GuardianEventCard({ event, darkMode = false }: GuardianEventCardProps) {
    ...
    // For guardians, we show if any of their children are registered
    ...
    return (
        <>
            {/* Link to guardian event detail page */}
            <Link href={`/dashboard/guardian/events/${event.id}`} className="group block">
