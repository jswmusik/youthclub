...
export default function GuardianNewsDetailPage() {
    ...
    if (loading) return (
        <div className={`min-h-screen ${darkMode ? 'bg-[var(--dark-800)]' : 'bg-[#F8F7FE]'}`}>
        ...
        <div className={`min-h-screen ${darkMode ? 'bg-[var(--dark-800)]' : 'bg-[#F8F7FE]'}`}>
            ...
            {isSidebarOpen && (
                <div className="fixed inset-0 z-50 lg:hidden">
                    ...
                    />
                    <div className={`absolute left-0 top-0 bottom-0 w-72 border-r p-4 overflow-y-auto ${darkMode ? 'bg-[var(--dark-800)] border-[var(--dark-600)]' : 'bg-white border-[#4D4DA4]/10'}`}>
            ...
            <div className={`w-full h-[50vh] sm:h-[60vh] relative ${darkMode ? 'bg-[var(--dark-800)]' : 'bg-[#F8F7FE]'}`}>
                {heroImageUrl ? (
                    <img 
                        src={heroImageUrl} 
                ...
                {/* Gradient Overlay */}
                <div className={`absolute inset-0 bg-gradient-to-t ${darkMode ? 'from-[var(--dark-800)] via-[var(--dark-800)]/50' : 'from-[#F8F7FE] via-[#F8F7FE]/50'} to-transparent`} />
                
                {/* Back Button - Positioned on Hero */}
                <button 
                    onClick={() => router.push('/dashboard/guardian/news')}
                    className={`absolute top-20 sm:top-24 left-4 sm:left-8 flex items-center gap-2 px-4 py-2 backdrop-blur-sm text-sm font-medium transition-all ${darkMode ? 'bg-[var(--dark-800)]/80 text-[var(--brand-light)] hover:bg-[var(--dark-800)]' : 'bg-white/80 text-gray-800 hover:bg-white'}`}
                >
            ...
            <div className="relative -mt-32 sm:-mt-40 z-10">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {/* Title */}
                    <h1 className={`text-3xl sm:text-4xl md:text-5xl font-bold mb-6 leading-tight font-heading ${darkMode ? 'text-[var(--brand-light)]' : 'text-gray-900'}`}>
                        {article.title}
                    </h1>

                    {/* Metadata Row */}
                    <div className={`flex flex-wrap items-center gap-4 sm:gap-6 py-6 border-y mb-8 ${darkMode ? 'border-[var(--dark-600)]' : 'border-[#4D4DA4]/15'}`}>
                        {/* Author */}
                        ...
                        {/* Divider */}
                        <div className={`hidden sm:block w-px h-10 ${darkMode ? 'bg-[var(--dark-600)]' : 'bg-[#4D4DA4]/15'}`} />
                    ...
                    {/* Excerpt/Lead */}
                    {article.excerpt && (
                        <p className={`text-lg sm:text-xl leading-relaxed mb-8 font-medium ${darkMode ? 'text-[var(--brand-light)]/80' : 'text-gray-700'}`}>
                            {article.excerpt}
                        </p>
                    )}
                    ...
                    {/* Article Content */}
                    <div 
                        className={`prose prose-lg max-w-none ${darkMode ? 'news-content-dark prose-invert' : 'news-content-light'}`}
                        dangerouslySetInnerHTML={{ __html: sanitizeHtml(article.content) }} 
                    />

                    {/* Footer */}
                    <div className={`mt-12 pt-8 border-t ${darkMode ? 'border-[var(--dark-600)]' : 'border-[#4D4DA4]/15'}`}>
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {article.tags_details.map(tag => (
                                    <span 
                                        key={tag.id} 
                                        className={`px-3 py-1 text-xs font-medium border ${darkMode ? 'bg-[var(--dark-700)] text-[var(--brand-light)]/70 border-[var(--dark-600)]' : 'bg-[#EBEBFE] text-gray-600 border-[#4D4DA4]/15'}`}
                                    >
                                        {tag.name}
                                    </span>
                                ))}
                            ...
                            <button
                                onClick={() => router.push('/dashboard/guardian/news')}
                                className={`flex items-center gap-2 font-medium text-sm transition-colors ${darkMode ? 'text-[var(--brand-primary)] hover:text-[var(--brand-primary)]/80' : 'text-[#4D4DA4] hover:text-[#3D3D8A]'}`}
                            >
                                <ArrowLeft className="w-4 h-4" />
                                {t('backToAllNews')}
                            </button>
