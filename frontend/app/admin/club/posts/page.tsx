...
import { Plus, Search, BarChart3, ChevronUp, Eye, Edit, Trash2, X, FileText, Users, UserPlus, TrendingUp } from 'lucide-react';
...
export default function ClubAdminPostsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            postsData = postsData.filter((p: any) => {
                const title = p.title || '';
                // Exclude activity posts: group joins, new groups, questionnaire completions, new questionnaires, inventory actions, rewards
            ...
            // Client-side filtering for scope - Club admins only see CLUB scope
            if (scope) {
                if (scope === 'CLUB') {
                    postsData = postsData.filter((p: any) => 
                        !p.is_global && 
                ...
                // If no scope filter, only show club posts (filter out global and municipality posts)
                postsData = postsData.filter((p: any) => 
                    !p.is_global && 
    ...
    const getScopeBadge = (post: any) => {
        // Club admins only see club posts
                ...
                <Card className="border border-gray-100 shadow-sm bg-white">
                    <div className="px-6 py-4 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                            ...
                            {/* Scope Filter - Club admins only see CLUB scope */}
                                ...
                                <Card key={post.id} className="overflow-hidden border-l-4 border-l-[#4D4DA4] shadow-sm">
                                    ...
                                    <CardContent className="space-y-3 pt-0">
                                        ...
                                        <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
                                            <Link href={buildUrlWithParams(`/admin/club/posts/${post.id}`)} className="flex-1">
                                            ...
                                            <Link href={buildUrlWithParams(`/admin/club/posts/edit/${post.id}`)} className="flex-1">
                                                <Button variant="ghost" size="sm" className="w-full justify-center gap-2 text-gray-600 hover:text-gray-900 hover:bg-gray-50">
