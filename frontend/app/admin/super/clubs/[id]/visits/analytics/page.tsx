...
import { BarChart3, Users, UserCheck, TrendingUp, Building2 } from 'lucide-react';
...
export default function SuperClubAnalyticsPage() {
  ...
  const tGenders = useTranslations('followers.genders');
  ...
  return (
    <div className="py-4 sm:py-8 px-0 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-4 sm:px-0">
        <BackButton href={`/admin/super/clubs/${clubId}`} translationKey="clubVisits.analytics.backToClub" />
      </div>
      ...
      <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h3 className="text-sm font-semibold text-[var(--brand-light)]">{t('timePeriod.title')}</h3>
            <p className="text-xs text-[var(--brand-light)]/50 mt-0.5">{t('timePeriod.description')}</p>
          </div>
      ...
      {loading ? (
        <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] p-8">
          <div className="py-12 text-center">
            <div className="inline-flex flex-col items-center gap-4">
              ...
              <span className="text-[var(--brand-light)]/60 animate-pulse">{t('loadingAnalytics')}</span>
            </div>
          </div>
        </div>
      ) : !stats ? (
        <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] p-8">
          <div className="py-12 text-center">
            ...
            <p className="text-[var(--brand-light)]/50 font-medium">{t('emptyState.noDataAvailable')}</p>
            <p className="text-sm text-[var(--brand-light)]/30 mt-1">{t('emptyState.dataWillAppear')}</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-0 sm:gap-6">
            ...
            <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] overflow-hidden group hover:border-[var(--brand-primary)]/30 transition-all">
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <div className="text-sm text-[var(--brand-light)]/50 font-medium">{t('stats.totalVisits')}</div>
              </div>
            </div>
            ...
            <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] overflow-hidden group hover:border-[var(--brand-blue)]/30 transition-all">
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <div className="text-sm text-[var(--brand-light)]/50 font-medium">{t('stats.uniqueYouth')}</div>
              </div>
            </div>
            ...
            <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] overflow-hidden group hover:border-[var(--brand-third)]/30 transition-all">
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <div className="text-sm text-[var(--brand-light)]/50 font-medium">{t('stats.avgVisitsPerYouth')}</div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] overflow-hidden">
              <div className="px-6 py-4 border-b border-[var(--dark-600)] bg-[var(--dark-700)]/50">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-[var(--brand-light)]">{t('charts.visitsTrend.title')}</h2>
                  </div>
                </div>
                <div className="p-6">
                  {stats.timeline && stats.timeline.length > 0 ? (
                    <>
                          ...
                          return (
                            <div key={day.date} className="flex-1 flex flex-col items-center group relative">
                              <div className="absolute bottom-full mb-2 hidden group-hover:block bg-[var(--dark-600)] text-[var(--brand-light)] text-xs p-2 rounded-lg whitespace-nowrap z-10 shadow-lg border border-[var(--dark-500)]">
                                ...
                                <div className="text-[var(--brand-primary)]">{day.count} {day.count === 1 ? t('charts.visitsTrend.visit') : t('charts.visitsTrend.visits')}</div>
                              </div>
                              <div 
                      ...
                      <div className="flex justify-between text-xs text-[var(--brand-light)]/40 mt-4 pt-4 border-t border-[var(--dark-600)]">
                        <span>{t('timePeriod.daysAgo', { count: period })}</span>
                        <span>{t('timePeriod.today')}</span>
                      </div>
                    </>
                  ) : (
                    <div className="h-64 flex items-center justify-center text-[var(--brand-light)]/40">
                      <div className="text-center">
                        ...
                        <p>{t('charts.visitsTrend.noDataAvailable')}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-[var(--dark-800)] rounded-none sm:rounded-2xl border-y sm:border border-[var(--dark-600)] overflow-hidden">
              <div className="px-6 py-4 border-b border-[var(--dark-600)] bg-[var(--dark-700)]/50">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-[var(--brand-light)]">{t('charts.genderDistribution.title')}</h2>
                  </div>
                </div>
                <div className="p-6">
                  {stats.demographics && stats.demographics.length > 0 ? (
                    <div className="space-y-5">
                      {stats.demographics.map((item: any) => {
                        ...
                        const genderCode = item.user__legal_gender?.toUpperCase();
                        
                        // Get translated gender label
                        const getGenderDisplay = (gender: string | null | undefined): string => {
                          if (!gender) return t('charts.genderDistribution.notSpecified');
                          const genderKey = gender.toUpperCase() as 'MALE' | 'FEMALE' | 'OTHER';
                          return tGenders(genderKey) || gender;
                        };
                        
                        const genderLabel = getGenderDisplay(item.user__legal_gender);
                        ...
                        return (
                          <div key={item.user__legal_gender || 'none'}>
                            <div className="flex justify-between text-sm mb-2">
                              <span className="font-semibold text-[var(--brand-light)]">{genderLabel}</span>
                            ...
                            <div className="w-full bg-[var(--dark-600)] rounded-full h-3 overflow-hidden">
                              <div 
                                className={`bg-gradient-to-r ${getGenderColor(item.user__legal_gender)} h-3 rounded-full transition-all`}
                                style={{ width: `${percent}%` }}
                              ></div>
                            </div>
                          </div>
                    ...
                    <div className="h-48 flex items-center justify-center text-[var(--brand-light)]/40">
                      <div className="text-center">
                        ...
                        <p>{t('charts.genderDistribution.noDataAvailable')}</p>
                      </div>
                    </div>
                  )}
                </div>
