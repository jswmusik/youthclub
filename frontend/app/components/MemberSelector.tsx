...
export default function MemberSelector({ criteria, selectedIds, onChange, excludeGroupId, darkMode = true }: MemberSelectorProps) {
  const t = useTranslations('groupsAdmin.form.memberSelector');
      ...
      <div className={`p-4 ${bgHeader} border-b ${borderColor}`}>
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-3">
            ...
            <h4 className={`font-semibold ${textColor}`}>{t('title')}</h4>
          </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span className="hidden sm:inline">{t('findMatches')}</span>
              </>
            )}
          </button>
        </div>
      ...
      <div className="max-h-80 overflow-y-auto">
        {!hasSearched && (
          <div className={`text-center py-12 ${textMuted}`}>
            ...
            <p className={`text-sm font-medium ${textColor} mb-1`}>{t('readyToSearch')}</p>
            <p className="text-xs">{t('clickToSearch')}</p>
          </div>
        )}
        ...
        {hasSearched && candidates.length === 0 && (
          <div className={`text-center py-12 ${textMuted}`}>
            ...
            <p className={`text-sm font-medium ${textColor} mb-1`}>{t('noMatchesFound')}</p>
            <p className="text-xs">{t('noEligibleUsers')}</p>
          </div>
        )}
        ...
        {candidates.length > 0 && (
          <div className="p-2">
            ...
            <div className={`flex justify-between items-center px-2 py-2 mb-2 rounded-lg ${darkMode ? 'bg-[var(--dark-600)]' : 'bg-gray-50'}`}>
              <span className={`text-xs ${textMuted}`}>{candidates.length} {t('usersFound')}</span>
              <div className="flex gap-2">
                <button 
                  type="button" 
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${textColor}`}>
                <span className="font-semibold">{selectedIds.length}</span> {selectedIds.length === 1 ? t('memberWillBeAdded') : t('membersWillBeAdded')}
              </span>
            </div>
            <button 
              type="button" 
