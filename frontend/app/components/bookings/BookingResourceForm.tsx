...
export default function BookingResourceForm({ initialData, redirectPath, clubId }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    allowed_group: typeof initialData?.allowed_group === 'object' ? initialData.allowed_group.id : (initialData?.allowed_group || ''),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        // Handle empty string for allowed_group - send empty string to clear it
      ...
      if (initialData) {
        ...
        await api.post('/bookings/resources/', data, config);
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
           ...
           <div>
              ...
              <input 
                ...
                onChange={e => setFormData({...formData, max_bookings_per_user_per_week: parseInt(e.target.value)})}
              />
           </div>
